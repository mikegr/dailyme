...
import com.dailyme.app.decodeBase64Content
import kotlinx.coroutines.launch

private data class LoadedFile(val sha: String, val content: String)

@Composable
fun FileViewScreen(state: AppState, api: GitHubApi, path: String) {
    ...
    LaunchedEffect(path, state.owner, state.repo, state.branch) {
        ...
        loaded = null
        try {
            val file = api.getFile(state.owner, state.repo, path, state.branch)
            val content = file.content?.let { decodeBase64Content(it) } ?: ""
            loaded = LoadedFile(file.sha, content)
            editedText = content
    ...
    fun performSave() {
        ...
        scope.launch {
            try {
                ...
                val refreshed = api.getFile(state.owner, state.repo, path, state.branch)
                val content = refreshed.content?.let { decodeBase64Content(it) } ?: editedText
                loaded = LoadedFile(refreshed.sha, content)
                editedText = content
                isEditing = false
                commitMessage = ""
