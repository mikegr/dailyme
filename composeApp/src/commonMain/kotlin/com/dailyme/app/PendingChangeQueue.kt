...
class PendingChangeQueue(private val store: KeyValueStore) {
    ...
    suspend fun all(): List<PendingChange> {
        ensureLoaded()
        return _changes.value
    }

    suspend fun find(owner: String, repo: String, branch: String, path: String): PendingChange? {
        ensureLoaded()
        return _changes.value.find { sameTarget(it, owner, repo, branch, path) }
    }

    suspend fun upsert(change: PendingChange) {
        ensureLoaded()
    ...
    suspend fun remove(owner: String, repo: String, branch: String, path: String) {
        ensureLoaded()
